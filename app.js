...
// //
// // The delete button should delete JUST the container that the button is in. The change button should change
// the background-color of the container to red. The default or 'normal' state of the background-color should
// be yellow. Clicking the button a second time should change it back to yellow, 3rd time red, 4th time yellow,
//  etc.
// //
...
// This assessment is not an indicator of how you will perform post Prime or a measure of
//  overall success thus far. It just gives us an idea of where you are at.
// //
...
function createListener(){
    ...
    $('.container').on('click','.generate',addButtonBox);
    $('.container').on('click','.delete', nukeBox);
    $('.container').on('click','.change', changeBox);
...
function nukeBox(){
    console.log("I am in nuke box");
    $(this).parent().remove();

}

function changeBox(){
    $(this).parent().toggleClass('change-red');
...
function addButtonBox(){
    ...
    console.log("in function to add buttons");
    $('.container').append('<div class = "button-box">I am button box</div>');
    var $el = $('.container').children().last();

    $el.append('<button class = "delete">delete</button>');
    $el.append('<button class = "change">change</button>');
    $el.append('<p class = "show-count">Generate Row Click Count: ' + rowCount + '</p>');
